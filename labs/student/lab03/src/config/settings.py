...
class Settings(BaseSettings):
    ...
    cache_jitter_ratio: float = Field(
        default=0.15,
        description="TTL 抖动比例，避免缓存雪崩"
    )

    # 分层 TTL（秒）
    ttl_embedding: int = Field(
        default=604800,  # 7天
        description="Embedding 层缓存 TTL"
    )
    ttl_retrieval: int = Field(
        default=600,  # 10分钟
        description="检索层缓存 TTL"
    )
    ttl_rerank: int = Field(
        default=600,  # 10分钟
        description="重排层缓存 TTL"
    )
    ttl_qa: int = Field(
        default=600,  # 10分钟
        description="QA 层缓存 TTL"
    )
