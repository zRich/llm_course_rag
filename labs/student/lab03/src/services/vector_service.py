...
class VectorService:
    ...
    def search_similar_chunks(self, query: str, 
        ...
        try:
            # 将查询文本向量化
            # TODO(lab03-lesson14): 为查询与检索接入缓存：
            # - Key示例：{"layer":"search","q":canonical(query),"doc_ids":sorted(document_ids)}；
            # - Value：检索候选的payload与score列表；
            # - TTL：热门1h，长尾10m，并加入抖动避免雪崩；
            # - 命中：直接返回缓存候选，跳过向量化与搜索。
