...
class QAService:
    ...
    def answer_question(self, question: str, 
        ...
        try:
            ...
            # TODO(lab03-lesson13): 构建可溯源引用(citations)。
            # - 需求：为每条答案引用生成结构化条目，包含chunk_id、document_id、
            #   原文片段、起止位置（可选），以及相似度/重排分数；
            # - 输出字段：`citations: List[{"chunk_id", "document_id", "text", "score"}]`；
            # - 验收：答案中出现的引用标号 [1],[2] 与 `citations` 对应。
            ...
            # TODO(lab03-lesson14): 为问答结果接入缓存：
            # - Key：{"layer":"qa","q":canonical(question),"doc_ids":sorted(document_ids)}；
            # - TTL：短期10~30m；命中返回，避免重复生成；
            # - 注意：当底层文档更新或向量重建时需失效。
            # TODO(lab03-lesson13): 在最终输出中加入结构化引用与可信度。
            # - 字段：`citations`（与sources关联）、`confidence`（0~1）；
            # - 计算：基于相似度/重排分数的加权与模型自评估；
            # - 验收：`confidence`在多个问题上可解释，并与引用数量相关。
    ...
    def _generate_answer(self, question: str, context_chunks: List[Dict]) -> Dict:
        ...
        try:
            ...
            # TODO(lab03-lesson13): 在提示词中加入“引用标注”要求。
            # - 模型需在输出中插入引用标号 [1],[2]，并在末尾给出引用列表；
            # - 引用内容来自 `context_chunks` 的 `source` 与 `content`。
...
如果文档内容与问题不相关，请礼貌地说明无法基于提供的内容回答该问题。"""
        # TODO(lab03-lesson13): 扩展为结构化输出要求（JSON格式）。
        # - 字段：`answer`, `citations`, `confidence`, `notes`；
        # - 模型需遵循格式输出，便于前端渲染与评估。
