...
class RerankService:
    ...
    def rerank(self, query: str, candidates: List[Dict], top_m: int = 10) -> List[Dict]:
        ...
        # TODO(lab03-lesson14): 为重排结果接入缓存：
        # - Key建议：{"layer":"rerank","q":canonical(query),"ids":[chunk_ids...]}
        # - Value：重排后的有序列表（含rerank_score）；TTL 10~30m，命中直接返回。
        # - 注意：候选变化时需失效；可用候选ID集合的hash作为key组成部分。
        return pre + tail