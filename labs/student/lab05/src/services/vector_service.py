...
class VectorService:
    ...
    async def reindex_all_documents(self, force: bool = True) -> Dict:
        ...
        Args:
            force: 是否强制重新向量化（删除并重建所有文档的向量）
        
        Returns:
            包含处理统计信息的字典
        """
        # TODO(lab05-lesson19): 实现全量重建
        # 目标：复用批量向量化逻辑（vectorize_documents），传入 force_revectorize=force
        # 步骤建议：
        # 1) 调用 self.vectorize_documents(document_ids=None, force_revectorize=force)
        # 2) 记录处理统计日志（processed_documents / processed_chunks）
        # 3) 返回结果字典（包含 processed_documents, processed_chunks, processing_time 等）
        # 提示：确保异常被记录并向上抛出，以便路由统一处理。
        raise NotImplementedError("请在 Lesson19 中实现 reindex_all_documents()")