...
async def search_vectors(
    ...
    try:
        ...
        # TODO(lab05-lesson20): 集成元数据过滤 DSL
        # 目标：如果 request.filters 存在，调用 apply_filters(raw_results, [f.dict() for f in request.filters])
        # 要求：任何过滤异常都应回退为未过滤结果 raw_results，不影响主流程。
        # 完成后请将下方占位实现替换为真实逻辑。
        results = raw_results
...
async def reindex_all_documents(
    ...
    """
    try:
        # TODO(lab05-lesson19): 调用服务层的 reindex_all_documents(force)，返回处理统计
        # 要求：将 request.force_revectorize 传递给服务，构造 VectorizeResponse。
        # 完成后请删除下方占位实现。
        raise NotImplementedError("请在 Lesson19 中完成 /vectors/reindex 路由实现")
