...
from api.dependencies import (
    ...
    get_upload_directory, cleanup_temp_file, get_vector_service
)
...
async def delete_document(
    ...
    try:
        ...
        # 先删除向量存储中的向量（级联清理）
        try:
            _ = vector_service.delete_document_vectors(int(document_id))
        except Exception:
            # 向量清理失败不阻塞文档删除，但记录日志
            logger.warning(f"级联删除向量失败，继续删除文档: {document_id}")
