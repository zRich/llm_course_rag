...
class DocumentProcessor:
    ...
    def process_document(self, file_path: Path, db: Session, 
        ...
        try:
            # TODO(lab01-task1): 实现文档解析逻辑
            # 任务说明：根据文件扩展名选择合适的解析器处理文档
            # 实现要点：
            # 1. 获取文件扩展名：file_extension = file_path.suffix.lower()
            # 2. 根据扩展名选择解析器：
            #    - 如果是 '.pdf'，使用 self.pdf_parser.parse_pdf(file_path)
            #    - 如果是 '.txt'，使用 self.txt_parser.parse_txt(file_path)
            #    - 其他格式抛出 ValueError("不支持的文件类型: {file_extension}")
            # 3. 检查解析结果：如果 parse_result["extraction_success"] 为 False，抛出异常
            # 期望返回：parse_result 字典，包含 extraction_success、cleaned_text 等字段
            ...
            raise NotImplementedError("请实现文档解析逻辑")
            
            # TODO(lab01-task1): 实现文档记录创建
            # 任务说明：根据解析结果创建Document数据库记录
            # 实现要点：
            # 1. 调用 self._create_document_record(parse_result, db) 创建文档记录
            # 2. 该方法会返回一个 Document 对象
            # 期望返回：document 对象
            
            # 示例代码：
            # document = self._create_document_record(parse_result, db)
            
            # TODO(lab01-task1): 实现文本分块处理
            # 任务说明：将解析后的文本进行分块处理
            # 实现要点：
            # 1. 使用 self.text_splitter.split_text() 方法
            # 2. 传入参数：
            #    - text: parse_result["cleaned_text"]
            #    - strategy: split_strategy
            #    - metadata: 包含 document_id、file_name、split_strategy 的字典
            # 期望返回：chunks_data 列表
            ...
            # TODO(lab01-task1): 实现分块记录创建
            # 任务说明：将分块数据保存到数据库
            # 实现要点：
            # 1. 调用 self._create_chunk_records(chunks_data, document.id, db)
            # 2. 该方法会返回 Chunk 对象列表
            # 期望返回：chunk_objects 列表
            
            # 示例代码：
            # chunk_objects = self._create_chunk_records(chunks_data, document.id, db)
            
            # TODO(lab01-task1): 更新文档统计信息
            # 任务说明：更新文档的统计信息，如分块数量等
            # 实现要点：
            # 1. 调用 self._update_document_stats(document, chunk_objects, db)
            # 2. 该方法会更新文档的统计字段
    ...
    def _create_document_record(self, parse_result: Dict, db: Session) -> Document:
        ...
        Returns:
            Document对象
        """
        # TODO(lab01-task1): 实现文档记录创建逻辑
        # 任务说明：根据解析结果创建并保存Document记录到数据库
        # 实现要点：
        # 1. 创建Document对象，设置以下字段：
        #    - id: 使用 str(uuid.uuid4()) 生成唯一ID
        #    - filename: parse_result["file_name"]
        #    - title: parse_result.get("title") 或 filename（去掉扩展名）
        #    - file_type: parse_result["file_type"]
        #    - file_size: parse_result["file_size"]
        #    - content_hash: parse_result["content_hash"]
        #    - processing_status: "processing"
        #    - created_at: datetime.utcnow()
        # 2. 将Document对象添加到数据库会话：db.add(document)
        # 3. 提交事务：db.commit()
        # 4. 刷新对象：db.refresh(document)
        # 期望返回：保存后的Document对象
        ...
        #     id=str(uuid.uuid4()),
        #     filename=parse_result["file_name"],
        #     title=parse_result.get("title") or Path(parse_result["file_name"]).stem,
        #     file_type=parse_result["file_type"],
        #     file_size=parse_result["file_size"],
        #     content_hash=parse_result["content_hash"],
        #     processing_status="processing",
        ...
        raise NotImplementedError("请实现文档记录创建逻辑")
