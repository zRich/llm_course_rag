...
class EmbeddingService:
    ...
    def _init_local_model(self):
        ...
        try:
            ...
            logger.info(f"本地嵌入模型加载成功，维度: {self.model.get_sentence_embedding_dimension()}")
    ...
    def _get_local_embedding(self, text: str) -> Tuple[List[float], Dict]:
        ...
        try:
            ...
            # TODO(lab01-task2): 实现本地模型向量化
            # 任务说明：使用本地sentence-transformers模型生成文本的嵌入向量
            # 实现要点：
            # 1. 使用 self.model.encode() 方法生成向量
            # 2. 参数设置：
            #    - text: 输入文本
            #    - convert_to_tensor=False: 返回numpy数组而不是tensor
            # 3. 将numpy数组转换为Python列表：embedding.tolist()
            # 4. 计算处理时间：processing_time = time.time() - start_time
            # 期望返回：(向量列表, 元数据字典)
            ...
            raise NotImplementedError("请实现本地模型向量化逻辑")
    ...
    def _get_api_embedding(self, text: str, model: str = None) -> Tuple[List[float], Dict]:
        ...
        try:
            ...
            # TODO(lab01-task2): 实现API模型向量化
            # 任务说明：使用火山引擎API生成文本的嵌入向量
            # 实现要点：
            # 1. 使用 self.client.embeddings.create() 方法
            # 2. 参数设置：
            #    - input: text
            #    - model: model or self.model_name
            # 3. 从响应中提取向量：response.data[0].embedding
            # 4. 计算处理时间：processing_time = time.time() - start_time
            # 期望返回：(向量列表, 元数据字典)
            ...
            raise NotImplementedError("请实现API模型向量化逻辑")
    ...
    def get_embeddings_batch(self, texts: List[str], model: str = None) -> Tuple[List[List[float]], List[Dict]]:
        ...
        try:
            # TODO(lab01-task2): 实现批量向量化逻辑
            # 任务说明：将文本列表分批处理，生成向量
            # 实现要点：
            # 1. 初始化结果列表：all_embeddings = [], all_metadata = []
            # 2. 按批次大小分割文本：
            #    for i in range(0, len(texts), self.batch_size):
            #        batch = texts[i:i + self.batch_size]
            # 3. 处理每个批次：
            #    batch_embeddings, batch_metadata = self._process_batch(batch, model or self.model_name)
            # 4. 合并结果：
            #    all_embeddings.extend(batch_embeddings)
            #    all_metadata.extend(batch_metadata)
            # 5. 添加延迟避免API限制：time.sleep(self.rate_limit_delay)
            # 期望返回：(所有向量列表, 所有元数据列表)
            ...
            #     
            #     # API限制延迟
            #     if self.provider == "api" and i + self.batch_size < len(texts):
            #         time.sleep(self.rate_limit_delay)
            
            raise NotImplementedError("请实现批量向量化逻辑")
