...
async def hybrid_search(
    ...
    try:
        ...
        raise HTTPException(status_code=501, detail="TODO(lab02-L8): 融合策略未实现，请完成 Lesson 8")
    except Exception:
        raise HTTPException(status_code=500, detail="融合阶段出现错误")

    # 4) 过滤（可选）
    # 4) 过滤（可选；未实现则返回 501）
    fused_filtered = fused
    if req.filters:
        try:
            fused_filtered = apply_filters(fused, [f.dict() for f in req.filters])
        except NotImplementedError:
            raise HTTPException(status_code=501, detail="TODO(lab02-L9): 过滤 DSL 未实现，请完成 Lesson 9")
        except Exception:
            raise HTTPException(status_code=500, detail="过滤阶段出现错误")
    ...
    if req.rerank_top_m and req.rerank_top_m > 0:
        try:
            ...
            raise HTTPException(status_code=501, detail="TODO(lab02-L10): 重排未实现，请完成 Lesson 10")
        except Exception:
